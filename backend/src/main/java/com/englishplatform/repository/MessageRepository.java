...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT DISTINCT m.sender FROM Message m WHERE m.receiver.id = :userId " +
           "UNION SELECT DISTINCT m.receiver FROM Message m WHERE m.sender.id = :userId AND m.receiver IS NOT NULL")
    List<Object> findContactUserIds(@Param("userId") Long userId);
